...
The initial terms of these sequences are
\[
  \begin{aligned}
    P_0^{(-1)} & = 0, & P_i^{(-1)} & = e_i, \\
    Q_0^{(-1)} & = 1, & Q_i^{(-1)} & = 0,
...
i.e.
\[
  P_i^{(n)} = P_i^{(n-1)} \text{ and } Q_i^{(n)} = Q_i^{(n-1)}.
\]
...
In this space, the pivot operation becomes a linear operation on the projective coordinates.
