...
similar to the definition of continued fractions.
This is followed by the two main results of this thesis.
...
We can derive a bottom-up definition based on the inverse pivot operation.
...
Then, $ℓ ∈ \{1, …, d\}$ is the pivot index of $x^{(n)}$,
...
function.
If one can guarantee that the new function satisfies the first requirement, the
convergence will still hold.
The requirement will only be needed in one place,
