...
In this section, we assume that we have an MDCF $[a^{(0)}, a^{(1)}, a^{(2)} …]$
...
\begin{proof}
  ...
  \begin{align*}
    ...
    & = x_i' - (λ₀' ε₀ + λ₁' ε₁ + λ_d' ε_d).
  \end{align*}
  However, as $n$ increases the values $εᵢ$ become arbitrarily small
  and because the values $λ₀', λ₁', …, λ_d$ all lie between $0$ and $1$,
