...
Given an infinite MDCF representation~$[a₀; a_1, …]$ of a vector $x ∈ ℝ^d$, we define the following:
...
So far in our analysis, we have implicitly assumed that the MDCF $[a^{(0)}; a^{(1)}, a^{(2)} …]$
...
Since we have used the Euclidean algorithm to construct the MDCF,
...
[a^{(1)}; a^{(2)}, …]$ which leads to the convergence of the whole MDCF.
Finally, we assume that the target vector $x = (x₁, …, x_d)$ is irrational,
which means that the MDCF is infinite.
...
\[
  0 < \frac{1}{a_{ℓ}^{(n)}} ≤ c
  \quad \text{ and } \quad
  0 ≤ \frac{a_i^{(n)}}{a_{ℓ}^{(n)}} ≤ c \quad \text{ for every } i ≠ ℓ,
\]
...
The second requirement then states that during the construction of the MDCF
...
So we need this specifically for MDCFs.

\begin{example}
  Consider the MDCF $[(2, 1); \overline{(2, 1)}]$.
  ...
  Therefore, the following proof won't apply to this MDCF.
\end{example}
...
\begin{lemma}
  ...
  \[
    r_i^{(n)} = λ₀ \frac{P_{i0}^{(n-1)}}{Q_0^{(n-1)}} + λ₁ \frac{P_{i1}^{(n-1)}}{Q_1^{(n-1)}} + ⋯ + λ_d \frac{P_{id}^{(n-1)}}{Q_d^{(n-1)}}.
  \]
\end{lemma}
...
\begin{proof}
  ...
  \begin{align*}
    ...
    & ≥ λ₀ s_i^{(n-1)} + λ₁ s_i^{(n-1)} + ⋯ + λ_d s_i^{(n-1)}
      = s_i^{(n-1)}.
  \end{align*}
...
coefficients $λ₀, λ₁, …, λ_d$ from Lemma~\ref{lem:conv-conv}.
...
\begin{proof}
  ...
  \[
    1 = λ₀ + λ₁ + ⋯ + λ_d ≤ λ_ℓ (1 + dc).
  \]
  Therefore, $λ_ℓ ≥ 1/M$ with $M = 1 + dc > 0$.
\end{proof}
...
which is the original vector $x$ used for construction of the MDCF.
...
\begin{proof}
  ...
  \begin{align*}
    x_i
    & = λ₀' \frac{P_{i0}^{(n-1)}}{Q_0^{(n-1)}}  + λ₁' \frac{P_{i1}^{(n-1)}}{Q_1^{(n-1)}} + λ_d' \frac{P_{id}^{(n-1)}}{Q_d^{(n-1)}} \\
    & = λ₀' (x_i' - ε₀) + λ₁' (x_i - ε₁) + ⋯ + λ_d' (x_d - ε_d) \\
    & = x_i' - (λ₀' ε₀ + λ₁' ε₁ + λ_d' ε_d).
  \end{align*}
  However, as $n$ increases the values $εᵢ$ must become arbitrarily small
  and because the coefficients $λ₀', λ₁', …, λ_d'$ all lie between $0$ and $1$,
...
For the generalization to multi-dimensional continued fractions,
...
\begin{proof}
  ...
  are contained in a field $ℚ(λ)$ with degree $≤ d+1$.
  Since $B^{(k)}$ consists solely of integers, every element in $x$ is contained in the same field $ℚ(λ)$.
  Therefore, they must also be algebraic numbers of degree $≤ d+1$.
\end{proof}
