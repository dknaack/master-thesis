...
\begin{lemma}
  ...
  There exists one index $ℓ^+ ∈ \{0, 1, …, d\}$ with $λ_{ℓ^+}^{(n)} > \frac{1}{M}$ for some constant $M > 0$.
\end{lemma}
...
\begin{proof}
  ...
  \[
    ...
    ≤ c,
  \]
  or equivalently $λᵢ^{(n)} ≤ c λ_{ℓ^+}^{(n)}$.
  But then,
  \[
    1 = λ₀^{(n)} + λ₁^{(n)} + ⋯ + λ_d^{(n)} ≤ λ_{ℓ^+}^{(n)} (1 + dc).
  \]
  Therefore, $λ_{ℓ^+}^{(n)} ≥ 1/M$ with $M = 1 + dc > 0$.
\end{proof}
