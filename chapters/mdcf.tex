...
Let $ℓ_n$ be the sequence of indices used in construction of the MCF.
The vector $x^{(n)}$ denotes the $n$-th complete quotient of $x$
and if $ℓ = ℓ_n$ is the index of the largest element in $x^{(n)}$,
...
\begin{enumerate}
  ...
  \item
    ...
    Formally, for every $N ≥ 0$, we can find an index $n ≥ N$ such that $ℓ_n = ℓ$.
  \item
    The distance between the same index is bounded.
    More formally:
    Then, for every $n ≥ 0$,
    there exists an $m > n$ such that $ℓ_n = ℓ_m$ and $m ≤ n + L$
...
largest element.
These requirements are already satisfied by the Jacobi-Perron algorithm,
but not by the generalized Euclidean algorithm.
So we need these specifically for MCFs.
...
\begin{lemma}
  \label{lem:conv-conv}
  For some $N ≥ 0$,
  there exist coefficients $λ₀^{(n)}, λ₁^{(n)}, …, λ_d^{(n)} ≥ 0$ for every $n ≥ N$
  such that $λ₀^{(n)} + λ₁^{(n)} + ⋯ + λ_d^{(n)} = 1$ and
  \[
...
\begin{lemma}
  ...
  Let $ℓ$ be the index of the largest element in $x^{(n)}$,
  then $λ_ℓ^{(n)} > 1/C$ for some positive integer $C$.
\end{lemma}
...
\begin{proof}
  ...
  For $Q_0^{(n-1)} = Q_ℓ^{(n-1)}$, this is trivial.
  But for the other values the issue is that $Q_i^{(n-1)}$ might be larger than $Q_ℓ^{(n-1)}$,
  if $i$ is the pivot index, for example.
  From the initial requirements it follows that after at most $L$ iterations,
  we must have $ℓ$ as the pivot index again.
  Therefore, there must have been some index $m$ before $n$ but after $n - L$
  where the $ℓ = \argmax_i x_i^{(m)}$.
  and $Q_ℓ^{(n-1)} = Q_ℓ^{(m)}$.
  ...
  \[
    ...
    ≥ \frac{1}{A} Q_ℓ^{(m)},
  \]
  where $k$ is the pivot index for $x^{(m+1)}$.
  We can repeat this step for every pivot index from $m$ to $n$
  tracing back a bound from $Q_ℓ^{(m)}$ to $Q_i^{(n-1)}$.
  ...
  of Equation~\ref{eq:lambda-ratio}.
