...
In a vector space, we can reduce any set of vectors $A = \{A₁, …, Aₙ\}$ with $n > d$
to a basis $B$ such that both sets span the same vector space
by taking $d$ linearly independent vectors from the set $A$.
In a lattice, this is not as simple.
Taking only $d$ linearly independent vectors can generate a different lattice than the initial set.
...
applications \cite{Ajtai96,Gentry08}. % TODO: Such as?
