...
In this thesis, we will mostly consider fields $ℚ(α)$ where the element $α$ is an algebraic number.
...
\begin{proof}
  ...
  Thus, $f_α$ is the unique monic polynomial for $α$.
\end{proof}
...
Using the minimal polynomial, we define the \emph{degree} of an algebraic
number as the degree of its minimal polynomial.
For example, $\sqrt[3]{2}$ would have degree $3$,
since its minimal polynomial is $X^3 - 2$.
Algebraic numbers of degree $2$ are called \emph{quadratic irrationals}
and algebraic numbers of degree $3$ are called \emph{cubic irrationals}.
