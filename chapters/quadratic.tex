...
\begin{lemma}
  The norm of the convergents $N(β_n)$ is bounded by some constant $c$ independent of $n$.
\end{lemma}
...
\begin{corollary}
  For every quadratic irrational $x$, there exists a matrix $U ∈ ℤ^{2×2}$ with
  determinant $1$, which has eigenvectors $(1, x)$ and $(1, \overline{x})$.
\end{corollary}
...
\begin{example}
  ...
  The matrix must have $(1, \sqrt{2})$ and $(1, -\sqrt{2})$ as eigenvectors,
  since it is a multiplication
