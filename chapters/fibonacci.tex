...
With the minimum strategy,
each element in the solution vector $x$ was greater than $1$ after one iteration.
With this strategy the exact opposite happens:
Each element in the solution vector is smaller than $1$ after one iteration,
except the element we pivot with.
The reason is that $\{x_ℓ\}$ is the largest fractional value,
so the ratio $\{x_i\}/\{x_ℓ\}$ will always be less than $1$.
However, $1/\{x_ℓ\}$ is still greater than $1$, because $\{x_ℓ\} < 1$.
