...
Let $\vec x = (ψ, 1 - ψ)$, where $ψ$ is the only real positive root of $x^2 + x - 1$.
For this solution, it does not matter which element we choose to pivot with, it will always
yield the same result.

Pivoting with $x_1$:
\begin{align*}
  \frac{1}{x_1} - 1 & = \frac{1}{ψ} - 1 = ψ, \\
  1 - \frac{x_2}{x_1} & = 1 - \frac{1 - ψ}{ψ} = 1 - ψ.
\end{align*}

Pivoting with $x_2$:
\begin{align*}
  \frac{1}{x_2} - 2 & = \frac{1}{1 - ψ} - 2 = ψ, \\
  1 - \frac{x_1}{x_2} & = 1 - \frac{ψ}{1 - ψ} = 1 - ψ.
\end{align*}
