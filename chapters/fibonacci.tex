...
\begin{lemma}
  ...
  with $p_ℓ/q$ with each having an irreducible fraction, then
  \[
    q = p_ℓ',
    \qquad
    p_ℓ = a_ℓ p_ℓ' + q',
...
\begin{theorem}
  Let $x = (p₁/q, …, p_d/q) ∈ ℚ^d$ be the solution vector after one iteration
  such that $0 < q < p₁ < … < p_d$.
  The generalized Euclidean algorithm requires at least $(n - d)$ steps for some $n ≥ d$ when taking the
  minimum fractional value at each step if
  \[
    q ≥ F(n),
    \qquad
    pᵢ ≥ \sum_{k = 0}^i F(n - k),
...
\begin{proof}
  ...
  \begin{align*}
    q ≥ F(d) = 1,
    \qquad
    p_i ≥ \sum_{k=0}^i F(d - k) = i + 1 \text{ for every } i ≤ d.
  \end{align*}
  These bounds are the smallest bounds we can have, since any smaller set of
  integers does not even satisfy $0 < q < p₁ < ⋯ < p_d$.
  Furthermore, any other set of numbers cannot require less than $0$ steps, so
  the theorem holds for $n = d$.

  % TODO: Fix indices
  Suppose the algorithm requires $n - d$ steps for some input $x' = (p₁'/q', …, p_d'/q')$.
  ...
  \begin{align*}
    p_1' & ≥ \sum_{k=0}^d F(n - k) = F(n + 1).
  \end{align*}
  Similarly, because all other values with $i ≠ 1$ are shifted one place to the
  left, we have
  \begin{align*}
    p_i' & ≥ \sum_{k=0}^{i-1} F(n - k) = \sum_{k=1}^i F(n + 1 - k) \text{ for } 2 ≤ i ≤ d.
  \end{align*}
  Let $x ∈ ℚ^d$ such that $x' = \mathrm{pivot}_ℓ(x)$ for some index $ℓ$ and $a = \floor{x}$.
  The smallest fractional value is $\{x_ℓ\}$ if and only if $1/\{x_i\}$ is the largest value.
  Since $p_1'/q'$ is the largest value, we pivot with $ℓ = 1$.
  ...
  \begin{align*}
    ...
    p_i
    & = a_i p_1' + p_i'
    ≥ F(n + 1) + ∑_{k=1}^i F(n + 1 - k)
    ≥ ∑_{k=0}^i F(n + 1 - k).
  \end{align*}
  % TODO: I feel like this is the wrong conclusion/direction.
  Therefore, $x$ requires at least $n+1$ steps.
\end{proof}
