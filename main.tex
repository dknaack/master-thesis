...
For the purposes of this thesis, I do not believe that Hermite's question is
relevant in the case of a simple pivoting strategy like choosing the minimum
element in each iteration.
My reasoning is that for the Euclidean algorithm, we need a minimal periodic
continued fraction, which is simply $[1; \overline{1}]$.
Therefore, for higher dimensions, we can equally look for a minimal periodic
solution which is unrelated to Hermite's question.

However, I suspect that Hermite's question \emph{is} relevant when looking for
the optimal pivoting strategy.
