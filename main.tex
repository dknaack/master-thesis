...
\[
  x_i = \frac{F(n - i) + a_{d-i+1} F(n - i + 1) + a_{d-i+2} F(n - i + 2) + \dots + a_d F(n)}{F(n)}.
\]
...
From these values we can construct a very simple linear system in the following way:
\[
  B_n =
  \begin{pmatrix}
    F(n) & 0 & \dots & 0 & 0 \\
...
\begin{example}
  ...
  which approximates the optimal solution from Equation~\ref{eq:rotate-solution} via
